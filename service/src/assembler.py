...
Instruction = collections.namedtuple('Instruction', ['opcode', 'destination_1', 'destination_2', 'literal_1', 'literal_2'])
...
OUTPUTD_DESTINATION = create_destination(((1<<28)-1), INPUT_ONE, MATCHING_ONE)
OUTPUTS_DESTINATION = create_destination(((1<<28)-2), INPUT_ONE, MATCHING_ONE)
REGISTER_INPUT_HANDLER_DESTINATION = create_destination(((1<<28)-3), INPUT_ONE, MATCHING_ONE)
DEREGISTER_INPUT_HANDLER_DESTINATION = create_destination(((1<<28)-4), INPUT_ONE, MATCHING_ONE)
...
def generate_header(constants: typing.List[DestinationToUpdate],
                    labels: typing.List[DestinationToUpdate],
                    external_references: typing.List[ExternalSymbol],
                    exported: typing.List[ExportedSymbol]) -> bytes:
    magic_bytes = b"sephiALD"
    num_constant = len(constants)
    num_to_fix = len(labels)
    num_external_ref = len(external_references)
    num_exported = len(exported)

    to_return = magic_bytes
    to_return += struct.pack('<HHHH',
                             num_constant,
                             num_to_fix,
                             num_external_ref,
                             num_exported)
    for constant in constants:
        to_return += constant.to_binary()

    for label in labels:
        to_return += label.to_binary()

    for external_reference in external_references:
        to_return += external_reference.to_binary()

    for export in exported:
        to_return += export.to_binary()
        ...
        for dest in ['destination_1', 'destination_2']:
            ...
            if destination_node.opcode == OPCODES['OUTD']:
                ...
                constants.append(DestinationToUpdate(node_to_idx[node],
                                                     is_first_destination=(dest == 'destination_1'),
                                                     is_second_destination=(dest == 'destination_2'),
                ))
                ...
                if destination_node.opcode.num_inputs == 1:
                    ...
                    assert(not (destination_node.input_1 and destination_node.input_2))
...
def parse_create_ir_graph(input: typing.TextIO) -> Graph:
    ...
    for line in input:
        ...
        # Frankly the syntax is such that this can be generalized and cleaned up for any instruction, but I don't have time for that. 
            ...
            true_branch = Node(OPCODES['DUP'], node_num, None, None, None, None, True)
            node_num += 1
            to_return.nodes.append(true_branch)
            variables[true_output].append(len(to_return.nodes) - 1)

            false_branch = Node(OPCODES['DUP'], node_num, None, None, None, None, True)
            node_num += 1
            to_return.nodes.append(false_branch)
            variables[false_output].append(len(to_return.nodes) - 1)

            node = Node(OPCODES[args[3]], node_num, input_var, test_var, Destination(true_branch, INPUT_ONE), Destination(false_branch, INPUT_ONE))
            node_num += 1
            to_return.nodes.append(node)
                ...
                for p in variables[input]:
                    ...
                    if parent.destination_1 is not None:
                        ...
                        if parent.destination_2 is not None:
                            ...
                            new_dup = Node(OPCODES['DUP'], node_num, None, None, parent.destination_2, None, True)
                            node_num += 1
                            parent.destination_2 = Destination(new_dup, INPUT_ONE)
...
def output_graph(graph, graph_output, output_file):
    graph = optimize_graph(graph)
    if (graph_output):
        with open(graph_output, 'w') as g:
            graph_to_dot(graph, g)

    instructions, constants, labels, external_references, exported = graph_to_instructions(graph)

    with open(output_file, 'wb') as f:
        output = serialize_instructions(instructions)

        header = generate_header(constants, labels, external_references, exported)
        f.write(header)
        f.write(output)
...
def main(input_file, output_file, graph_output):
    ...
    output_graph(graph, graph_output, output_file)
